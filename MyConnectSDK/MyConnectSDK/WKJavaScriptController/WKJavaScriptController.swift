...
//  BuildArchitect
//
//  Created by CuongTQ26 on 27/10/2022.
//
...
open class WKJavaScriptController: NSObject {
    ...
    //                readOnlyProperties.append(bridge) // ******* Cuong note: bridge func need 'return'
    //                continue
    //            }
    //            source += """
    ...
    private func bridgeScript() -> WKUserScript {
        ...
        for bridge in bridges {
            if bridge.argumentCount == 0, bridge.isReturnRequired {
                readOnlyProperties.append(bridge) // - MARK: CUONG NOTE: validate -> bridge func need 'return' *****
                continue
            }
            // - MARK: CUONG NOTE: gen group func generation *****
            source += """
                \(bridge.jsSelector): function() {
                    const id = \(name).\(ReserveKeyword.createUUID)();
                    const args = Array.from(arguments).concat(id);
        ...
        source += "};"
        for bridge in readOnlyProperties { // - MARK: CUONG NOTE: group func need 'return' *****
            source += """
                Object.defineProperty(\(name), '\(bridge.jsSelector)', {
                    key: '\(bridge.jsSelector)',
                    get: function get() {
